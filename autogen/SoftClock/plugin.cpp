...
static std::array<FMOD_DSP_PARAMETER_DESC*, 8> g_param_ptrs;
static std::array<FMOD_DSP_PARAMETER_DESC, 8> g_param_store;
...
FMOD_RESULT F_CALL plugin_create(FMOD_DSP_STATE *st) {
  ...
  st->plugindata = ps;
  auto &reg =
      AirwinRegistry::registry[AirwinRegistry::nameToIndex["SoftClock"]];
...
F_EXPORT FMOD_DSP_DESCRIPTION *F_CALL FMODGetDSPDescription() {
  ...
  std::strncpy(g_desc.name, "SoftClock", sizeof(g_desc.name) - 1);
