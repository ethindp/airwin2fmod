...
if(FMOD_FOUND AND NOT TARGET FMOD::FMOD)
  ...
  if(WIN32)
    ...
    set_target_properties(
      FMOD::FMOD
      PROPERTIES IMPORTED_IMPLIB "${FMOD_LIBRARY}"
    ...
    if(CMAKE_CONFIGURATION_TYPES)
      foreach(CONFIG ${CMAKE_CONFIGURATION_TYPES})
        ...
        set_target_properties(
          FMOD::FMOD
          PROPERTIES IMPORTED_IMPLIB_${CONFIG_UPPER}
                     "${FMOD_LIBRARY_DIR}/${CONFIG_LIB_NAME}"
                     IMPORTED_LOCATION_${CONFIG_UPPER}
                     "${FMOD_LIBRARY_DIR}/${CONFIG_DLL_NAME}")
      endforeach()
    endif()
  ...
  elseif(APPLE)
    if(CMAKE_SYSTEM_NAME STREQUAL "iOS"
      ...
      set_target_properties(
        FMOD::FMOD
        PROPERTIES IMPORTED_LOCATION "${FMOD_LIBRARY}"
                   INTERFACE_INCLUDE_DIRECTORIES "${FMOD_INCLUDE_DIRS}")
    else()
      # macOS uses dylib
      if(FMOD_IS_DEBUG)
      ...
      set_target_properties(
        FMOD::FMOD
        PROPERTIES IMPORTED_LOCATION "${FMOD_RUNTIME_LIBRARY}"
                   INTERFACE_INCLUDE_DIRECTORIES "${FMOD_INCLUDE_DIRS}")
    endif()
  ...
  if(WIN32
    ...
    set_property(
      TARGET FMOD::FMOD
      APPEND
      PROPERTY INTERFACE_LINK_LIBRARIES FMOD::FMOD_Runtime)
  endif()

  # For Linux, set RPATH handling
  if(UNIX AND NOT APPLE)
    set_target_properties(FMOD::FMOD PROPERTIES INTERFACE_LINK_DIRECTORIES
                                                "${FMOD_LIBRARY_DIR}")
    # Suggest RPATH settings to users
    set(FMOD_SUGGESTED_RPATH
        "${FMOD_LIBRARY_DIR}"
        CACHE STRING "Suggested RPATH for FMOD library")
      ...
      if(target_type STREQUAL "EXECUTABLE")
          ...
          if(CMAKE_CONFIGURATION_TYPES)
            ...
            add_custom_command(
              ...
              COMMAND ${CMAKE_COMMAND} -E copy_if_different
                      "${FMOD_RUNTIME_LIBRARY}" "$<TARGET_FILE_DIR:${target}>"
        ...
        elseif(UNIX)
          ...
          add_custom_command(
            ...
            POST_BUILD
            COMMAND
              ${CMAKE_COMMAND} -E copy_if_different "${FMOD_RUNTIME_LIBRARY}"
          ...
          if(NOT "${RUNTIME_NAME}" STREQUAL "${SYMLINK_BASE}")
            add_custom_command(
              TARGET ${target}
              POST_BUILD
              COMMAND ${CMAKE_COMMAND} -E create_symlink "${RUNTIME_NAME}"
  ...
  # Automatically apply to all future executable targets This uses CMake's
  # deferred call feature (3.19+)
  if(CMAKE_VERSION VERSION_GREATER_EQUAL "3.19")
    ...
    message(
      STATUS
        "FMOD: For automatic DLL copying with CMake < 3.19, call fmod_enable_auto_copy() after find_package(FMOD)"
    )
  endif()
endif()
