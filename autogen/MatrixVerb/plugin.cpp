...
static std::array<FMOD_DSP_PARAMETER_DESC*, 7> g_param_ptrs;
static std::array<FMOD_DSP_PARAMETER_DESC, 7> g_param_store;
...
FMOD_RESULT F_CALL plugin_create(FMOD_DSP_STATE *st) {
  ...
  st->plugindata = ps;
  auto &reg =
      AirwinRegistry::registry[AirwinRegistry::nameToIndex["MatrixVerb"]];
...
F_EXPORT FMOD_DSP_DESCRIPTION *F_CALL FMODGetDSPDescription() {
  ...
  std::strncpy(g_desc.name, "MatrixVerb", sizeof(g_desc.name) - 1);
